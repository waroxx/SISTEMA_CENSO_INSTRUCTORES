function TIPO_otroTipo(campos, otro) {
    const tipo_otroTipo =
      '<div class=" form-group col-md-3" style="text-align:center" > <select class="selectpicker seltipo" id="seltipo" required="required"> {{OPTIONS}} </select> <br/><br/><div class="form-group otrotipo" id="otrotipo" style="display:block"><input type="text" value="' +
      otro +
...
function TIPO_default(campos, selected) {
    const tipo_default =
      '<div class="form-group col-md-3" style="text-align:center" > <select class="selectpicker seltipo" id="seltipo" required="required"><option value="0">Seleccione</option>{{OPTIONS}} </select> <br/><br/><div class="form-group otrotipo" id="otrotipo" style="display:none"><input type="text" class="form-control otrotipoI" style=" width:220px; margin-left:35px"/></div></div>';
    let options = "";
    campos.map(function (item) {
        let op = '<option value="' + item.id + '">' + item.Actividad + "</option>";
        if (item.id === selected) {
...
function DESC_default(campos, selected) {
    const desc_default =
      '<div class="col-md-3" style="text-align:center"> <select type="text" class="form-control descripcion" id="desc" required="required"><option value="0">Seleccione</option> {{OPTIONS}} </select></div>';
    let options = "";
    campos.map(function (item) {
        let op =
...
function TEMA_visible(campos, selected) {
    const tema_visible =
      '<div class="col-md-3" style="text-align:center"> <select class="selectpicker seltema" required="required"><option value="0">Seleccione</option> {{OPTIONS}} </select></div>';
    let options = "";
    campos.map(function (item) {
...
function TEMA_invisible(campos, selected) {
    const tema_invisible =
      '<div class="col-md-3" style="text-align:center;visibility:hidden"> <select class="selectpicker seltema" required="required"><option value="0">Seleccione</option> {{OPTIONS}} </select></div>';
    let options = "";
    campos.map(function (item) {
        let op = '<option value="' + item.id + '">' + item.tema + "</option>";
        if (item.id === selected) {
